...
const VisionMissionCard: React.FC<VisionMissionCardProps> = ({ title, description, imageSrc, listItems }) => {
      return (
            <div
                  className="rounded-2xl p-10 relative bg-cover h-[400px] overflow-hidden cursor-pointer w-full shadow-lg"
                  style={{ backgroundImage: "url('/image/home/bg-card-shop.svg')" }}
            >
                  <h4 className="text-[32px] text-white font-black">{title}</h4>
                  <div className="text-white mt-3 max-w-[480px]">
                        {listItems ? (
                              <ul className="list-decimal ps-5 text-base">
                  ...
                  <div className="absolute bottom-0 right-0 overflow-hidden">
                        <Image src={imageSrc} alt={title} width={title === "VISI" ? 200 : 160} height={title === "VISI" ? 200 : 160} />
                  </div>
            </div>
      );
};
      ...
      return (
            <div className="flex flex-col gap-8">
                  <div className="flex flex-col gap-2 text-center">
                        <h5 className="font-bold text-2xl text-neutral-600">Driving Quality and Trust</h5>
