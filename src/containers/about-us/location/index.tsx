...
const contactDetails: ContactInfo[] = [
      { icon: <IoLocationOutline size={30} color="#CC0000" className="w-9 h-9 md:w-[30px] md:h-[30px]"/>, text: "Jl. Raya Cilegon, Serdang, Kec. Kramatwatu, Kab. Serang, Banten 42161" },
      ...
      return (
            <div className="flex flex-col lg:flex-row justify-between items-center gap-10 md:gap-16">
                  <div className="flex flex-col gap-3">
                        <h3 className="text-2xl md:text-[32px] font-extrabold text-primary-500">Our Location</h3>
                        ...
                        {contactDetails.map((contact, index) => (
                              contact.href ? (
                                    <Link href={contact.href} key={index} className="flex gap-3 items-center" target="_blank" rel="noopener noreferrer">
                                          {contact.icon}
                                          <p className="text-sm md:text-lg text-black">{contact.text}</p>
                                    </Link>
                              ) : (
